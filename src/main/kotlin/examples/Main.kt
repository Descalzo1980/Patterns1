...
How I wonder what you're at!"""
 ...
 """.trimIndent()
    ...
    val myFun = listOf("hellO wOrlD", null, "fRom", null, "kOtlin")
    firstBig(myFun)

}

fun firstBig(string: List<String?>): List<String?> {
    val result = string.map {
        it?.lowercase(Locale.getDefault())?.replaceFirstChar { it.uppercase(Locale.getDefault()) }
    }
/*    val finalResult = if (result.isNullOrEmpty()) {
        result.map { it ?: "Skip" }
    } else {
        result
    }*/
    val finalResult = result.map {
        if (it.isNullOrEmpty()) "Skip" else it
    }
    println(finalResult)
    return finalResult
}
